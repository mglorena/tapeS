...
class TapeMailer {
     ...
     * @type string
     */
    var $From = "infotape@obras.unsa.edu.ar";
...
//        echo "<pre>".$header . $body . "</pre>"; // debugging
    ...
    function smtp_send($header, $body) {
        ...
        if (!$connection) {
            ...
            //printf("SMTP Error: could not connect to SMTP host server(s) %s", $hosts[$index]); //debug only
            return false;
        }
        ...
        if ($this->SMTPAuth) {
            if (!$smtp->Authenticate($this->Username, $this->Password)) {
                ...
                //printf("%s error de autenticacion<br>", $hosts[$index]); //debug only
                return false;
            }
        }
        ...
        if (!$smtp->Mail(sprintf("<%s>", $smtp_from))) {
            ...
            // printf("%s error de address<br>", $hosts[$index]); //debug only
            return false;
        }
...
//print "errr";
    ...
    function create_body() {
       ...
       // echo "<br/>messsage type :" . $this->message_type;
        switch ($this->message_type) {
            case "alt":
                // Return text of body
            ...
            case "attachments":
                ...
                if (!$body[] = $this->attach_all()) {
                    /*echo " sale por aqui 4";*/
                    return false;
                }
		/*echo "sale por attachemnts<br/>\n";*/
                break;
            case "alt_attachments":
