...
_WebSitePath = _WebSitePath.substring(
  0,
  _WebSitePath.indexOf(rootDir + "/", 0) + 5
);
console.log(_WebSitePath);
var _iBrowserHeight;
var _iBrowserWidth;
if (document.all) {
  ...
  try {
    ...
    //            var iBrowserHeight = $(window).height();  var iBrowserWidth = $(window).width();
  }
}
/***********/
var dates = {
...
function arrayAnios() {
  ...
  // Agrega los años anteriores al array
  for (let i = currentYear - 4; i < currentYear; i++) {
    years.push([i, i.toString()]);
  }

  // Agrega el año actual al array
  years.push([currentYear, currentYear.toString()]);

  // Agrega los años siguientes al array
...
function SendJsError(obj, location, data) {
  ...
  try {
    ...
    if (obj.message) {
      ...
      if (obj.description)
        sExtraData +=
          "<b>Description</b>: " + parseString(obj.description) + "<br>";
      if (obj.arguments) {
    ...
    try {
      if (data) {
        if (data.url) {
          ...
          sExtraData +=
            "<li><b>url</b>: " + parseString(data.url) + "<br></li>";
          try {
            sExtraData +=
              "<li><b>xmlHttp</b>: " +
              parseString(var_export(data.xmlHttp)) +
              "<br></li>";
          } catch (e) {}
          sExtraData +=
            "<li><b>isRunning</b>: " +
            parseString(data.isRunning) +
            "<br></li>";
          sExtraData += "</ul>";
        } else
          sExtraData +=
            "<b>Object params</b>: " + parseString(var_export(data)) + "<br>";
      } else
        sExtraData +=
          "<b>Object params - data is false</b>: " +
          parseString(var_export(data)) +
          "<br>";
    } catch (e) {
      alert(e.message);
    }
    ...
    console.log(
      parseString(sMessage),
      parseString(location),
      parseString(sExtraData)
    );
    x_SendJsError(
      parseString(sMessage),
      parseString(location),
      parseString(sExtraData),
      SendJsError_callback
    );
...
function var_export(mixed_expression, bool_return) {
  ...
  if (type === null) {
    ...
    if (
      iret.indexOf("dateFormat") == -1 &&
      outerIndent.indexOf("dateFormat") == -1
    )
      retstr = outerIndent + "array (\n" + iret + "\n" + outerIndent + ")";
    else retstr = outerIndent + date;
  } else if (type === "function") {
    funcParts = mixed_expression
      .toString()
      .match(/function .*?\((.*?)\) \{([\s\S]*)\}/);
    retstr =
      "create_function ('" +
    ...
    retstr =
      typeof mixed_expression !== "string"
        ? mixed_expression
        : "'" +
...
function writeLog(sText) {
  if (getId("dvLog")) {
    ...
    logDiv.setAttribute(
      "onclick",
      "javascript:document.documentElement.removeChild(this)"
    );
    document.documentElement.appendChild(logDiv);
  }
}

function SendJsError_callback(response) {
  //alert(response);
  console.log("SendJsError response:" + response);
}
