/**
 * @class Ext.ux.grid.PrinterLicenciasPersonal
 ...
 * Ext.require([
 *   'Ext.ux.grid.GridPrinterLicenciasPersonal',
 * ]);
 * 
 * 2 - Declare the Grid 
 ...
 * 
 * 3 - Print!
 * Ext.ux.grid.PrinterLicenciasPersonal.mainTitle = 'Your Title here'; //optional
...
Ext.define("Ext.ux.grid.PrinterLicenciasPersonal", {
    ...
    statics: {
        ...
        print: function (grid) {
            ...
            grid.store.data.each(function (item, row) {
                ...
                for (var key in item.data) {
                    ...
                    if (key === "Total")
                    {
                        value = item.data["LicA"];
                    }
                    if (key === "Fin")
                    {
                        item.data["Total"] = item.data["LicA"];
                        ;
                        var final = new Date(item.data['Inicio']);
                        var finalday = new Date(final.getFullYear(), final.getMonth(), (final.getDate() + parseInt(item.data["Total"])));
                        //console.log((final.getMonth() + 1));
                        //console.log((final.getDate() + parseInt(item.data["Total"])));
            ...
            //console.log("estoy en planilla de impresion");
            ...
            if (this.stylesheetPath === null) {
                var scriptPath = Ext.Loader.getPath('Ext.ux.grid.PrinterLicenciasPersonal');
                this.stylesheetPath = scriptPath.substring(0, scriptPath.indexOf('PrinterLicenciasPersonal.js')) + 'gridPrinterCss/printlicense.css';
            }
            ...
            var htmlMarkup = [
                ...
                '{% if (xindex % 25 === 0) { %}',
                '</table>',
                '<div style="font-size:8px;">', this.footerText, '</div>',
        ...
        bodyTpl: [
            ...
            '<td  style="{["text-align:"]}{[Ext.String.createVarName(values.align)]}{[values.width ? ";width:"+ values.width +"px" : ";width:30px"]}">\{{[Ext.String.createVarName(values.dataIndex)]}\}</td>',
            '<tpl else>',
