...
function LoadGridOnly($meses, $anio)
{
    try {
        ...
        $horaex = $horaexSearch->SearchRepByDep($meses, $anio);
        $response = array("1" => $horaex);
        return $response;
    } catch (Exception $ex) {
        $error = new Errors();
        $error->SendErrorMessage($ex, 'horaexrepdep.php - LoadGridOnly', $e);
    }
}
...
<?php
echo '<script type="text/javascript" src="horaexrepdep.js?' . Conf::VERSION . '"></script>';
?>
