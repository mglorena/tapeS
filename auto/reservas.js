        ...
        },
        {
          ...
          handler: function () {
            ...
            Ext.ux.grid.Printer.mainTitle =
              "Lista de Reservas <br/> Mes: " + mesn + "    Año : " + anio;
            Ext.ux.grid.Printer.headerText =
              "Universidad Nacional de Salta <br/> Dirección General de Obras y Servicios";
...
function GetChVe(evt) {
  try {
    ...
    if (fInicio !== null && fFin !== null && nPas !== "") {
      ...
      x_GetDisponibles(
        fInicio.f("yyyy-MM-dd"),
        fFin.f("yyyy-MM-dd"),
        nPas,
        reId,
        GetDisponibles_callback
      );
    }
  } catch (e) {
...
function GetDisponibles_callback(response) {
  try {
    ...
    var choferes = null;
    if (record) choferes = record.data["ChoferesIds"].split(",");
...
function FormField(record) {
  ...
  try {
    ...
    if (record) {
      if (record.data["Estado"] === "Pendiente") es = 1;
      else if (record.data["Estado"] === "Confirmada") es = 2;
      else if (record.data["Estado"] === "Suspendida") es = 3;
      else es = 4; // Realizada
      ...
      var url1 =
        "https://maps.google.com/maps?saddr=Salta,+Argentina&daddr=" +
        dest +
        ",+Argentina";
        ...
        listeners: {
          blur: function (obj) {
            ...
            var url1 =
              ...
              dest +
              ",+Argentina";
            Ext.getCmp("linkmap").setValue(
              "<a href='" + url1 + "' target='_blank'>Google Maps.</a>"
            );
          },
        },
      },
...
function CostoTotal() {
  try {
    var totalViaticos =
      Ext.getCmp("txtViaticos1").getValue() * dias +
      Ext.getCmp("txtViaticos2").getValue() * dias;
    var maxCH = Math.max(
      Ext.getCmp("txtVHEC1I").getValue(),
      Ext.getCmp("txtVHEC1V").getValue()
    );
    ...
    var costoVehiculo =
      (km * ccmb * parseFloat(pcmb).toFixed(2)).toFixed(2) +
      parseFloat(Ext.getCmp("txtPeaje").getValue()).toFixed(2);
    var hci = parseFloat(Ext.getCmp("txtHorasIda").getValue());
    var hcv = parseFloat(Ext.getCmp("txtHorasVue").getValue());
    var horasC = 0;
    if (Ext.getCmp("chkII").getValue()) horasC = hci;
    else {
      if (hci > 7) horasC = hci - 7;
    }

    if (Ext.getCmp("chkIV").getValue()) horasC += hcv;
    else {
      if (hcv > 7) horasC = hcv - 7;
    }
    var suma = parseFloat(horasC) * parseFloat(maxCH);
    var total =
      parseFloat(totalViaticos) + parseFloat(suma) + parseFloat(costoVehiculo);
...
function ImprimirCosto() {
  ...
  try {
    ...
    html +=
      ...
      dias +
      " días x " +
      (typeof choferes[1] !== "undefined" ? 2 : 1) +
      " chofer/es a $" +
      Ext.getCmp("txtViaticos1").getValue() +
      " aprox ";
    html += "</td>";
    ...
    var costoIdaHoraExCh1 = parseFloat(
      Ext.getCmp("txtHEC1I").getValue() * Ext.getCmp("txtVHEC1I").getValue()
    ).toFixed(2);
    var costoVueHoraExCh1 = parseFloat(
      Ext.getCmp("txtHEC1V").getValue() * Ext.getCmp("txtVHEC1V").getValue()
    ).toFixed(2);
    var maxCH = Math.max(
      Ext.getCmp("txtVHEC1I").getValue(),
      Ext.getCmp("txtVHEC1V").getValue()
    ...
    var costoIdaHoraExCh2 = parseFloat(
      Ext.getCmp("txtHEC2I").getValue() * Ext.getCmp("txtVHEC2I").getValue()
    ).toFixed(2);
    var costoVueHoraExCh2 = parseFloat(
      Ext.getCmp("txtHEC2V").getValue() * Ext.getCmp("txtVHEC2V").getValue()
    ).toFixed(2);
    ...
    html +=
      ...
      horasC +
      "hs por $" +
      parseFloat(maxCH).toFixed(2);
    html += "</td>";
    html +=
      "<td style='text-align:right'>$" + parseFloat(suma).toFixed(2) + "</td>";
    html += "</tr>";
    ...
    html += "</td>";
    html +=
      "<td style='text-align:right;height:25px;font-weight:bold;font-size:14px'>" +
      Ext.getCmp("txtTotal").getValue() +
      "</td>";
    html += "</tr>";
    ...
    var w = window.open(
      "Costo Aproximado",
      "",
      "width=450,height=400,resizeable,scrollbars"
    );
...
function InfoPresupuesto_callback(response) {
  try {
    ...
    Ext.getCmp("txtFechaInicio").setValue(
      FechaCompleta(new Date(feIni)) + " a las " + response[5]
    );
    Ext.getCmp("txtFechaFin").setValue(
      FechaCompleta(new Date(feFin)) + " a las " + response[7]
    );
    ...
    Ext.getCmp("txtChofer1").setValue(
      choferes[0]["LastName"] + ", " + choferes[0]["FirstName"]
    );
    if (typeof choferes[1] !== "undefined")
      Ext.getCmp("txtChofer2").setValue(
        choferes[1]["LastName"] + ", " + choferes[1]["FirstName"]
      );
    else {
    ...
    dias = Math.abs(
      Math.floor((s1.getTime() - l1.getTime()) / (1000 * 60 * 60 * 24))
    );
              ...
              if (form.isValid()) {
                form.submit({
                  ...
                  success: function (fp, o) {
                    msg(
                      "Success",
                      'Archivo "' + o.result.file + '" subido al servidor'
                    );
                    x_LoadReservas(
                      dia,
                      mes,
  ...
  try {
    ...
    if (action === "add") {
      ...
      title =
        "Editar Reserva : " +
        (record
          ?  record.data["ReservaId"] + " - " + record.data["Solicitante"] + " - " + record.data["Destino"]
          : "");
    ...
    var win = Ext.create("Ext.window.Window", {
      ...
      items: {
        ...
        url: "file-upload.php",
        fileUpload: true,
        bodyStyle: "padding: 5spx;border:none;",
        items: formFields,
        method: "post",
        buttons: [
          {
...
function DeleteReserva(record, grid, rowId) {
  try {
    if (hasUpdate) {
      ...
      Ext.Msg.confirm(
        "Eliminar Reserva",
        "Esta seguro que quiere eliminar este Reserva?",
        function (button) {
          if (button === "yes") {
...
function DeleteReserva_callback(response) {
  if (response) {
    ...
    SendJsError(
      new Error("Error DeleteReserva-reservas.js"),
      "reservas.js",
      response
    );
  }
...
function SaveReserva_callback(response) {
  if (response) {
    ...
    SendJsError(
      new Error("Error SaveReserva-reservas.js"),
      "reservas.js",
      response
    );
  }
...
function GrabarCosto_callback(response) {
  if (response) {
    ...
    SendJsError(
      new Error("GrabarCosto_callback-reservas.js"),
      "reservas.js",
      response
    );
  }
