...
//////////////////////
    ...
    orgChart.prototype.countNodes = function()
    {
        return nodes.length;
    };
    orgChart.prototype.getNode = function(id)
    {
        if (id === undefined) {
            id = '';
        }
    ...
    orgChart.prototype.allNodes = function()
    {
        var a = [], obj;
        var i;
        for (i = 0; i < nodes.length; i++) {
            obj = [nodes[i].id, nodes[i].txt, nodes[i].parent];
            a.push(obj);
        }
        return a;
    }
    ;
    orgChart.prototype.changeNode = function(id, pid, text)
    {
        if (id === undefined) {
            id = '';
        }
    ...
    orgChart.prototype.removeNode = function(id)
    {
        if (id === undefined) {
            return;
        }
        ...
        for (i = 0; i < nodes.length; i++) {
            ...
            if (nodes[i].id !== id && nodes[i].parent !== id) {
                allnodes[j] = nodes[i];
                j++;
            }
        }
...
// Internal functions:
//////////////////////
    orgChart.prototype.clearCanvas = function(idCanvas)
    {
    ...
    orgChartClick = function(event, offsetx, offsety)
    {
        ...
        if (i >= 0) {
            ...
            if (nodes[i].url.length > 0) {
                ...
                i3 = nodes[i].url.indexOf('popin');
                if (i1 >= 0 && i2 > i1) {
                    window.open(nodes[i].url);
                } else {
                    if (i3 >= 0)
                    {
                        var f = "PopinOptions(event,'" + nodes[i].id + "','" + nodes[i].parent + "','" + nodes[i].txt + "');";
                        func = nodes[i].url.replace("popin", f);
                        eval(func);
                    }
                    else {
