...
DROP PROCEDURE IF EXISTS `reservas_getById`  ;
CREATE DEFINER=`root`@`localhost` PROCEDURE `reservas_getById`(
...
FROM `Reservas`INNER JOIN Vehiculos ON Vehiculos.VehiculoId = Reservas.VehiculoId  
